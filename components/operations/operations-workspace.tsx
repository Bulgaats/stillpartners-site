...
function ClientInvoicesPanel({ data, isPending, runAction, today }: { data: OperationsWorkspaceData; isPending: boolean; runAction: (action: () => Promise<OperationsActionResult>) => void; today: string }) {
  ...
  const periodEnd = addIsoDays(periodStart, 13);
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
    <section className="dashboard-card">
      <div className="dashboard-card-header"><div className="dashboard-icon"><FileText className="size-5" /></div><div><h2 className="dashboard-card-title">Create 14-day client invoice</h2><p className="dashboard-muted">Only open records are included. Included records lock automatically.</p></div></div>
      <div className="grid gap-4"><Field label="Client"><select value={clientId} onChange={(event) => changeClient(event.target.value)}><option value="">Select client</option>{data.clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></Field><Field label="Period start"><input value={periodStart} onChange={(event) => setPeriodStart(event.target.value)} type="date" /></Field><div className="rounded-lg bg-gray-100 px-3 py-3 text-sm"><span className="font-bold text-gray-600">Period end:</span> <span className="font-black text-blue-950">{formatDate(periodEnd)}</span></div>
      ...
      <button className="dashboard-button dashboard-button-orange mt-4 w-full" disabled={isPending || !clientId || projectIds.length === 0 || entries.length === 0 || Number(rate) <= 0} onClick={submit} type="button">{isPending ? "Creating…" : "Create and lock invoice"}</button>
    </section>
    <section className="dashboard-card"><div className="dashboard-card-header"><div className="dashboard-icon"><DollarSign className="size-5" /></div><div><h2 className="dashboard-card-title">Client invoices</h2><p className="dashboard-muted">Private finance view for Bulgaa and Zaya.</p></div></div><div className="grid gap-3">{data.clientInvoices.map((invoice) => { const client = data.clients.find((item) => item.id === invoice.clientId); return <article className="rounded-xl border border-gray-200 p-4" key={invoice.id}><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black text-blue-950">{invoice.invoiceNumber}</p><p className="mt-1 text-sm text-gray-600">{client?.name} · {formatDate(invoice.periodStart)}–{formatDate(invoice.periodEnd)}</p><span className="mt-2 inline-block rounded-full bg-blue-100 px-2.5 py-1 text-xs font-bold text-blue-800">{invoice.status}</span></div><div className="sm:text-right"><p className="text-xl font-black text-blue-950">{formatMoney(invoice.totalAmount)}</p><p className="text-xs text-gray-500">GST {formatMoney(invoice.gstAmount)}</p><a className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-sm font-bold text-blue-950 hover:bg-gray-50" href={`/api/operations/client-invoices/${invoice.id}/pdf`}><Download className="size-4" /> PDF</a></div></div></article>; })}</div>{data.clientInvoices.length === 0 ? <EmptyState text="No client invoices have been created yet." /> : null}</section>
  </div>;
}
